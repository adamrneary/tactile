(function() {
  var buttonGroup, chart, columnData, frameVal, groupButton, leaderboardData, linesData, paretoData, stackButton, timeSlider, toColumn, toLeaderboard, toLines, toPareto;
  ...
  timeSlider = $("<div>").attr("id", "slider").attr("class", "ui-horizontal-slider");
  ...
  frameVal = [new Date(2012, 1, 1).getTime(), new Date(2012, 10, 1).getTime()];
  ...
  toLeaderboard.click(function(e) {
    var generateData,
      _this = this;
    e.stopPropagation();
    $("#below-chart").html('');
    generateData = function(count) {
      ...
      while (i < count) {
        ...
        index = 0;
        while (j < count) {
          if (leaderboardData[j].value > leaderboardData[i].value) {
            index++;
          }
          j++;
        }
    ...
    chart.data(generateData(4)).addSeries([
      {
        name: "enemies",
  ...
  toColumn.click(function(e) {
    e.stopPropagation();
    $("#below-chart").html('');
    chart.data(columnData).setXFrame(frameVal).axes({
      x: {
        dimension: 'time',
        frame: frameVal
      },
    ...
    chart.render();
    $("#below-chart").prepend(timeSlider);
    timeSlider.slider({
      min: 0,
      max: 11,
      values: [1, 10],
    ...
    return $("#below-chart").prepend(buttonGroup);
  });

  toLines.click(function(e) {
    e.stopPropagation();
    $("#below-chart").html('');
    chart.data(linesData).setXFrame(frameVal).axes({
      x: {
        dimension: "time",
        frame: frameVal
      },
    ...
    chart.data(paretoData).addSeries([
      {
        ...
        tooltip: function(d) {
          return "" + d.name + ": " + ((d.y * 100).toFixed(2)) + "% (cumulative)";
        }
      }
    ], {
      overwrite: true
    }).setAutoScale(true).setMinXFrame(1).axes({
      y: {
        dimension: "linear"
      },
      y1: {
