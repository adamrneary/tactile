(function() {
  var baseRenderer, buttonGroup, chart, chartData, currentChart, frameVal, leaderboardData, paretoData, stack, stackGroup, timeSlider, toArea, toColumn, toLeaderboard, toLines, toPareto, toWaterfall, unstack,
  ...
  baseRenderer = function() {
    var axes;
    chart.data(chartData).addSeries([
      {
        name: "reach actual",
    ...
    axes = _.keys(chart.initAxes());
    if (!((__indexOf.call(axes, "x") >= 0) && (__indexOf.call(axes, "y") >= 0))) {
      chart.axes({
        x: {
          dimension: 'time',
          frame: frameVal
